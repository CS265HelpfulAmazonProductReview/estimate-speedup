...
start = time.time()
def random_tune(df, parameter):
    trainset, validationset = df.randomSplit([0.8, 0.2], seed=205)
    gbt = GBTClassifier(labelCol='label', featuresCol="features", maxIter=500, maxDepth=parameter)
    pipeline = Pipeline(stages=[trainset.select("label"), trainset.select("features"), gbt])
    model = pipeline.fit(trainset)
    prediction = model.transform(validationset)
...
parameters = [2, 4, 8, 16, 32, 64]

pool.map(lambda parameter: random_tune(data_prepared_df, parameter), parameters)
